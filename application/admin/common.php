 ...
 * @param $key 要检测的内容
 */
function WSTCheckLoginKey($val)
{
    if ($val == '') return WSTReturn("登录账号不能为空");
    if (!WSTCheckFilterWords($val, config('registerLimitWords'))) {
        return WSTReturn("登录账号包含非法字符");
    }
    $dbo = Db::name('user')->where(["loginName" => ['=', $val], 'dataFlag' => 1]);
    $rs = $dbo->count();
    /*$dbo = Db::name('user')->select();
    dump($dbo);exit;*/
    if ($rs == 0) {
